...
| D2 | Capture via a temporary `arlog_i!` block inside `test_full_pipeline_pose`, gated by `env_logger::try_init()`, run with `RUST_LOG=info`. Print pose + error, then update constants and remove the block. | Standalone capture binary; `eprintln!`; `arlog_e!`/`arlog_w!`. | Per CLAUDE.md §2 logging convention, regen capture is a one-shot informational event, which maps to `arlog_i!`. `arlog_e!` is for misconfiguration / wiring errors and `arlog_w!` for recoverable anomalies — neither fits a diagnostic dump. Keeping capture in the same test file ensures we record exactly what the asserted code path produces (no drift between capture rig and assertion site). *(Brainstorming initially landed on `arlog_e!`; corrected during PR review.)* |
