...
/*
 ...
 - --patt-ratio F
   ...
   - --border-color black|white
       Choose the fill color used when the tool adds a border. Default is 'black'.
       When you request an automatic or explicit border, the program logs (stderr) the applied border in pixels
       and its source: explicit (from --border), auto (computed from --patt-ratio), or none.
