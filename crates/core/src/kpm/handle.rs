...
use crate::kpm::backend::{FeaturePoint, FreakMatcherBackend, KpmError, Point3d};
use crate::kpm::types::{
    KpmInputDataSet, KpmRefDataSet, KpmResult, DB_IMAGE_MAX, FREAK_SUB_DIMENSION,
};
use crate::types::ARParamLT;
use std::sync::Arc;
...
/// the C++ FFI backend ([`CppFreakMatcher`](crate::kpm::CppFreakMatcher)) can
...
/// [`Arc`]. `Arc` (Atomically Reference Counted) is a
...
///
/// ```rust,ignore
/// use webarkitlib_rs::kpm::{CppFreakMatcher, KpmHandle, KpmProcMode};
///
...
pub struct KpmHandle {
    ...
    ///
    /// Wrapped in [`Arc`] (Atomically Reference Counted)
...
impl KpmHandle {
    ...
    ///   (e.g. [`CppFreakMatcher`](crate::kpm::CppFreakMatcher)).
    ///
    /// # Defaults
    ///
...
mod tests {
    use super::*;
    use crate::kpm::backend::{FeaturePoint, Match, Point3d, QueryResult};
