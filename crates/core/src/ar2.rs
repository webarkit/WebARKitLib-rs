...
use crate::types::{ARParamLT, ARdouble, ARPixelFormat};
...
impl AR2Handle {
    pub fn new(xsize: i32, ysize: i32, pix_format: ARPixelFormat) -> Self {
        Self {
            ...
            templ: vec![AR2Template {
                xts1: 16, xts2: 16, yts1: 16, yts2: 16,
                xsize: 33, ysize: 33,
                ..Default::default()
            }; AR2_THREAD_MAX],
...
pub fn ar2_screen_coord_2_marker_coord(
    ...
    if let Some(lt) = cparam_lt {
        ...
        if m.abs() < 1e-10 { return Err("Singular matrix in coord conversion"); }
...
pub fn ar2_set_template_sub(
        ...
        for i in -templ.xts1..=templ.xts2 {
            ...
            let res = if let Some(lt) = cparam_lt {
                match lt.param_ltf.observ2ideal(ix2 as f32, iy2 as f32) {
                    Ok((isx, isy)) => ar2_get_image_value(cparam_lt, trans, image, isx, isy, blur_level),
    ...
    if k == 0 { return Err("No valid pixels in template"); }
...
pub fn ar2_select_template(
    ...
    if num == 0 {
        ...
        for (i, cand) in candidate.iter().enumerate() {
            if cand.flag == -1 { break; }
            if cand.flag != 0 { continue; }
            if cand.sx < (xsize / 8) as f32 || cand.sx > (xsize * 7 / 8) as f32
                || cand.sy < (ysize / 8) as f32 || cand.sy > (ysize * 7 / 8) as f32 {
                continue;
            }
        ...
        if get_vector_angle(pos[0], pos[1], &mut p2sinf, &mut p2cosf).is_err() { return -1; }
        if get_vector_angle(pos[0], pos[2], &mut p3sinf, &mut p3cosf).is_err() { return -1; }
        ...
        for (i, cand) in candidate.iter().enumerate() {
            ...
            pos[3][0] = cand.sx;
            pos[3][1] = cand.sy;
            if get_vector_angle(pos[0], pos[3], &mut p4sinf, &mut p4cosf).is_err() { continue; }

            let q1: usize;
            let r1: usize;
            let r2: usize;

            if (p3sinf * p2cosf - p3cosf * p2sinf >= 0.0) && (p4sinf * p2cosf - p4cosf * p2sinf >= 0.0) {
                if p4sinf * p3cosf - p4cosf * p3sinf >= 0.0 {
                    q1 = 1; r1 = 2; r2 = 3;
                } else {
                    q1 = 1; r1 = 3; r2 = 2;
                }
            } else if (p4sinf * p3cosf - p4cosf * p3sinf >= 0.0) && (p2sinf * p3cosf - p2cosf * p3sinf >= 0.0) {
                if p4sinf * p2cosf - p4cosf * p2sinf >= 0.0 {
                    q1 = 2; r1 = 1; r2 = 3;
                } else {
                    q1 = 2; r1 = 3; r2 = 1;
                }
            } else if (p2sinf * p4cosf - p2cosf * p4sinf >= 0.0) && (p3sinf * p4cosf - p3cosf * p4sinf >= 0.0) {
        ...
        for p_cand in prev_feature.iter_mut() {
            if p_cand.flag == -1 { break; }
            if p_cand.flag != 0 { continue; }
            p_cand.flag = 1;
            
            for (i, cand) in candidate.iter_mut().enumerate() {
                if cand.flag == -1 { break; }
                if cand.flag == 0 && p_cand.snum == cand.snum && p_cand.level == cand.level && p_cand.num == cand.num {
                    cand.flag = 1;
                    return i as i32;
                }
            }
...
pub fn get_vector_angle(p1: [f32; 2], p2: [f32; 2], psinf: &mut f32, pcosf: &mut f32) -> Result<(), &'static str> {
    ...
    if l == 0.0 { return Err("Zero length vector"); }
    *psinf = (p2[1] - p1[1]) / l;
    *pcosf = (p2[0] - p1[0]) / l;
    Ok(())
}
...
pub fn ar2_get_search_point(
    ...
    if ox1.is_none() {
        search[0][0] = -1; search[0][1] = -1;
        search[1][0] = -1; search[1][1] = -1;
        search[2][0] = -1; search[2][1] = -1;
        return;
    }
...
pub fn ar2_tracking_2d_sub(
    ...
    if ((templ.vlen * templ.vlen) as f32)
        < ((templ.xsize * templ.ysize) as f32 * 2.0 * 2.0) { // AR2_DEFAULT_TRACKING_SD_THRESH placeholder
    ...
    if surface_set.cont_num == 1 {
        ar2_get_search_point(cparam_lt, Some(wtrans1), None, None, feature_coord, &mut search);
    } else if surface_set.cont_num == 2 {
        ar2_get_search_point(cparam_lt, Some(wtrans1), wtrans2, None, feature_coord, &mut search);
    } else {
        ar2_get_search_point(cparam_lt, Some(wtrans1), wtrans2, wtrans3, feature_coord, &mut search);
    }
    ...
    result.pos3d[0] = surface.trans[0][0] * feature_coord.mx + surface.trans[0][1] * feature_coord.my + surface.trans[0][3];
    result.pos3d[1] = surface.trans[1][0] * feature_coord.mx + surface.trans[1][1] * feature_coord.my + surface.trans[1][3];
    result.pos3d[2] = surface.trans[2][0] * feature_coord.mx + surface.trans[2][1] * feature_coord.my + surface.trans[2][3];
...
pub fn ar2_tracking(
    ...
    *err = 0.0;
    ...
    for i in 0..surface_set.surface.len() {
        crate::math::mat_mul_fff(&surface_set.trans1, &surface_set.surface[i].trans, &mut handle.wtrans1[i]);
        if surface_set.cont_num > 1 {
            crate::math::mat_mul_fff(&surface_set.trans2, &surface_set.surface[i].trans, &mut handle.wtrans2[i]);
        }
        if surface_set.cont_num > 2 {
            crate::math::mat_mul_fff(&surface_set.trans3, &surface_set.surface[i].trans, &mut handle.wtrans3[i]);
        }
    }
    ...
    if handle.tracking_mode == AR2_TRACKING_6DOF {
        extract_visible_features(cparam_lt, &handle.wtrans1, surface_set, &mut handle.candidate, &mut handle.candidate2).map_err(|_| -1)?;
    } else {
        extract_visible_features_homography(handle.xsize, handle.ysize, &handle.wtrans1, surface_set, &mut handle.candidate, &mut handle.candidate2).map_err(|_| -1)?;
    }
    ...
    while i < handle.search_feature_num {
        ...
        let mut k = ar2_select_template(
            if candidate_ptr_idx == 0 { &mut handle.candidate } else { &mut handle.candidate2 },
        ...
        if k < 0 {
            if candidate_ptr_idx == 0 {
                candidate_ptr_idx = 1;
                k = ar2_select_template(&mut handle.candidate2, &mut surface_set.prev_feature, num, &mut pos, handle.xsize, handle.ysize);
                if k < 0 { break; }
            } else {
                break;
            }
        ...
        let cand = if candidate_ptr_idx == 0 { &handle.candidate[k as usize] } else { &handle.candidate2[k as usize] };
    ...
    if handle.tracking_mode == AR2_TRACKING_6DOF {
        unsafe {
            ...
            *err = ar2_get_trans_mat(icp_handle, surface_set.trans1, &handle.pos2d, &handle.pos3d, num as usize, trans, false);
            if *err > handle.tracking_thresh {
                crate::icp::icp_set_inlier_probability(icp_handle, 0.8);
                *err = ar2_get_trans_mat(icp_handle, *trans, &handle.pos2d, &handle.pos3d, num as usize, trans, true);
                if *err > handle.tracking_thresh {
...
pub fn ar2_get_trans_mat(
    ...
    use crate::icp::{ICPDataT, ICP2DCoordT, ICP3DCoordT};
    ...
    for j in 0..3 {
        for i in 0..3 { init_mat[j][i] = init_conv[j][i] as ARdouble; }
    }
    init_mat[0][3] = (init_conv[0][0] * dx + init_conv[0][1] * dy + init_conv[0][2] * dz + init_conv[0][3]) as ARdouble;
    init_mat[1][3] = (init_conv[1][0] * dx + init_conv[1][1] * dy + init_conv[1][2] * dz + init_conv[1][3]) as ARdouble;
    init_mat[2][3] = (init_conv[2][0] * dx + init_conv[2][1] * dy + init_conv[2][2] * dz + init_conv[2][3]) as ARdouble;
    
    let mut mat = [[0.0 as ARdouble; 4]; 3];
    let mut err = 0.0;
    
    if !robust_mode {
        crate::icp::icp_point(icp_handle, &data, &init_mat, &mut mat).map(|e| err = e as f32).ok();
    } else {
        crate::icp::icp_point_robust(icp_handle, &data, &init_mat, &mut mat).map(|e| err = e as f32).ok();
    }
    
    for j in 0..3 {
        for i in 0..3 { conv[j][i] = mat[j][i] as f32; }
    }
    conv[0][3] = (mat[0][3] - mat[0][0] * dx as ARdouble - mat[0][1] * dy as ARdouble - mat[0][2] * dz as ARdouble) as f32;
    conv[1][3] = (mat[1][3] - mat[1][0] * dx as ARdouble - mat[1][1] * dy as ARdouble - mat[1][2] * dz as ARdouble) as f32;
    conv[2][3] = (mat[2][3] - mat[2][0] * dx as ARdouble - mat[2][1] * dy as ARdouble - mat[2][2] * dz as ARdouble) as f32;
...
pub fn ar2_get_best_matching(
    ...
    for ii in 0..3 {
        ...
        let px = (search[ii][0] / (SKIP_INTERVAL + 1)) * (SKIP_INTERVAL + 1) + (SKIP_INTERVAL + 1) / 2;
        let py = (search[ii][1] / (SKIP_INTERVAL + 1)) * (SKIP_INTERVAL + 1) + (SKIP_INTERVAL + 1) / 2;
    ...
    for ii in 0..3 {
        if search[ii][0] < 0 {
            if ret { return Err("No valid search point"); }
            break;
        }
        ...
        for j in (py - ry..=py + ry).step_by((SKIP_INTERVAL + 1) as usize) {
            ...
            for i in (px - rx..=px + rx).step_by((SKIP_INTERVAL + 1) as usize) {
                ...
                if mf_image[(j * xsize + i) as usize] != 0 { continue; }
                mf_image[(j * xsize + i) as usize] = 1;
                
                let mut wval = 0;
                if ar2_get_best_matching_sub_fine(img, xsize, ysize, pix_format, mtemp, i, j, &mut wval).is_ok() {
                    ret = false;
...
pub fn ar2_get_best_matching_sub_fine(
    ...
    match pix_format {
        ARPixelFormat::MONO | ARPixelFormat::NV21 | ARPixelFormat::FourTwoZeroV | ARPixelFormat::FourTwoZeroF => {
            let ssx = -mtemp.xts1;
            let eex = mtemp.xts2;
            let ssy = -mtemp.yts1;
            let eey = mtemp.yts2;
            ...
            for j in ssy..=eey {
                ...
                for _i in ssx..=eex {
                    let pixel_val = img[row_start as usize + (_i - ssx) as usize * AR2_TEMP_SCALE as usize] as i32;
...
fn update_candidate(
    ...
    let m = if *keep_num == KEEP_NUM { KEEP_NUM - 1 } else { *keep_num };
    if *keep_num < KEEP_NUM { *keep_num += 1; }
